...
struct Options {
  ...
  // Up to two write buffers may be held in memory at the same time,// memtable and immemtable
  // so you may wish to adjust this parameter to control memory usage.
  // Also, a larger write buffer will result in a longer recovery time
  // the next time the database is opened.
  //
  // Default: 4MB
  size_t write_buffer_size; //memtable超过这个内存大小的话就会写table
  ...
  CompressionType compression; //压缩类型，不压缩或snappy压缩
...
struct ReadOptions {
  ...
  // Default: false
  bool verify_checksums; //读取的时候进行checksum校验
